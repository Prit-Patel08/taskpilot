...
export async function fetchCompanies(search?: string): Promise<string[]> {
  ...
  const body = await apiRequest<{ companies?: string[] }>(
    `/v1/jobs/companies${qs.toString() ? `?${qs.toString()}` : ""}`,
  );
...
export async function fetchMetrics(): Promise<MetricsSummary> {
  const body = await apiRequest<{ metrics: MetricsSummary }>(
