...
from app.core.rabbitmq import (
    SCORING_QUEUE_NAME,
    SCORING_RETRY_ROUTING_KEY_PREFIX,
    create_scoring_channel,
...
class ScoringConsumer:
    ...
    async def start(self, connection: AbstractRobustConnection) -> None:
        self._channel, self._queue, self._exchange = await create_scoring_channel(connection)
    ...
    async def _handle_message(self, message: AbstractIncomingMessage) -> None:
        ...
        try:
            with tracer.start_as_current_span(
                "scoring.message.receive",
                kind=SpanKind.CONSUMER,
                context=parent_context,
            ) as receive_span:
                receive_span.set_attribute("messaging.system", "rabbitmq")
                receive_span.set_attribute("messaging.operation", "process")
                receive_span.set_attribute("messaging.destination", SCORING_QUEUE_NAME)
                receive_span.set_attribute("messaging.destination.name", SCORING_QUEUE_NAME)
                try:
                    ...
                    event = ScoreCalculateEvent.model_validate(raw_payload)
                    _log(
                        "job_received",
                        request_id=request_id,
                        application_id=application_id,
                    ...
                    await asyncio.wait_for(
                        process_scoring_event(event, attempt=attempt),
