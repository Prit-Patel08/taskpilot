...
from aio_pika.abc import AbstractIncomingMessage, AbstractRobustChannel, AbstractRobustConnection, AbstractRobustQueue
...
def _apply_retry_metadata_to_span(
    ...
    *,
    ...
    span.set_attribute(
        "retry.error_message_short",
        classification.error_message_short,
    )
    span.set_attribute(
        "retry.error_fingerprint",
...
class ApplicationConsumer:
    ...
    async def start(self, connection: AbstractRobustConnection) -> None:
        self._channel, self._queue, self._exchange = await create_application_channel(connection)
    ...
    async def _schedule_retry(
        ...
        *,
        ...
        retry_routing_key = get_retry_routing_key(next_retry_count)
        ...
        with tracer.start_as_current_span(
            "application.retry.schedule",
            kind=SpanKind.PRODUCER,
        ) as retry_span:
            retry_span.set_attribute("messaging.system", "rabbitmq")
            retry_span.set_attribute("messaging.destination", retry_routing_key)
            retry_span.set_attribute("messaging.destination.name", retry_routing_key)
            retry_span.set_attribute("event.type", metric_event_name)
            retry_span.set_attribute("application.id", application_id)
            retry_span.set_attribute("retry.count", retry_count)
            retry_span.set_attribute("retry.max", MAX_RETRIES)
            retry_span.set_attribute("retry.scheduled", True)
            retry_span.set_attribute("retry.next_count", next_retry_count)
            retry_span.set_attribute("retry.delay_seconds", retry_delay_seconds)
            ...
            try:
                ...
                retry_span.record_exception(exc)
                retry_span.set_status(Status(StatusCode.ERROR))
                raise
    ...
    async def _handle_processing_failure(
        ...
        *,
        ...
        if classification.retryable and retry_count < MAX_RETRIES:
            receive_span.set_attribute("retry.scheduled", True)
            try:
                ...
                retry_schedule_classification = classify_exception(retry_exc)
                _apply_retry_metadata_to_span(
                    receive_span,
                    classification=retry_schedule_classification,
                    retry_header_corrupted=retry_header_corrupted,
                )
                receive_span.record_exception(retry_exc)
                ...
                _log(
                    ...
                    event_id=event_id,
                    attempt=attempt,
                    error_message=retry_schedule_classification.error_message_short,
                    retry_count=min(retry_count + 1, MAX_RETRIES),
                    retry_max=MAX_RETRIES,
                    retry_reason=retry_schedule_classification.reason,
                    retry_classification=retry_schedule_classification.classification,
                    retry_error_code=retry_schedule_classification.error_code,
                    retry_error_type=retry_schedule_classification.error_type,
                    retry_error_message_short=retry_schedule_classification.error_message_short,
                    retry_error_fingerprint=retry_schedule_classification.error_fingerprint,
                    retry_header_corrupted=retry_header_corrupted,
                )
                await message.nack(requeue=True)
                return

        receive_span.set_attribute("retry.scheduled", False)
    ...
    async def _handle_message(self, message: AbstractIncomingMessage) -> None:
        ...
        try:
            with tracer.start_as_current_span(
                ...
                receive_span.set_attribute("messaging.destination", APPLICATION_QUEUE_NAME)
                receive_span.set_attribute("messaging.destination.name", APPLICATION_QUEUE_NAME)
                receive_span.set_attribute("retry.count", retry_count)
                receive_span.set_attribute("retry.max", MAX_RETRIES)
                receive_span.set_attribute("retry.scheduled", False)
                receive_span.set_attribute("retry.header_corrupted", retry_header_corrupted)
                ...
                try:
                    ...
                    event = ApplicationCreatedEvent.model_validate(raw_payload)

                    receive_span.set_attribute("application.id", application_id)
                    receive_span.set_attribute("event.type", event.event_name)
                    ...
                    with tracer.start_as_current_span(
                        ...
                        processing_span.set_attribute("application.id", application_id)
                        processing_span.set_attribute("messaging.system", "rabbitmq")
                        processing_span.set_attribute("messaging.operation", "process")
                        processing_span.set_attribute("event.type", event.event_name)
                        processing_span.set_attribute("retry.count", retry_count)
                        processing_span.set_attribute("retry.max", MAX_RETRIES)
                        processing_span.set_attribute("retry.scheduled", False)
                        processing_span.set_attribute(
                            "retry.header_corrupted",
                            retry_header_corrupted,
                        )
                        try:
                            result = await asyncio.wait_for(
                                process_application_created(event, attempt=attempt),
                                timeout=MAX_PROCESSING_SECONDS,
                            )
                            processing_span.set_attribute("idempotent.skip", result.skipped)
                            if result.skip_reason is not None:
                                processing_span.set_attribute(
                                    "idempotent.skip_reason",
                                    result.skip_reason,
                                )
                            processing_span.set_attribute(
                            ...
                            receive_span.set_attribute("idempotent.skip", result.skipped)
                            if result.skip_reason is not None:
                            ...
                            processing_span.record_exception(exc)
                            processing_span.set_status(Status(StatusCode.ERROR))
                            raise
